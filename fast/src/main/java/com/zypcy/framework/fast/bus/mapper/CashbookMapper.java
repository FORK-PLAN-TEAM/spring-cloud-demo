...
public interface CashbookMapper extends BaseMapper<Cashbook> {
    ...
    @Select("select record_time,cash_type,dict_id,sum(amount) as amount from bus_cashbook where create_userid=#{createUserId} and date_format(record_time,'%Y%m%d')=#{sTime}\n" +
